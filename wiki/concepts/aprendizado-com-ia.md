---
title: "Aprendizado com IA"
type: concept
tags: [aprendizado, claude, educação, cursos, certificação]
source_count: 3
last_updated: 2026-04-23
---

# Aprendizado com IA

> **Fontes:** 3 | **Domínio:** Educação e desenvolvimento profissional com IA

## Definição

Uso de LLMs como tutores personalizados e curadoria de recursos para aprender habilidades de forma acelerada e estruturada.

## O que foi documentado

### Claude como tutor pessoal
[[evolving-ai]] documenta 6 prompts para aprendizado estruturado:
- Aprender nova skill em 20 horas com estrutura de tutor
- Trilha do básico ao avançado
- Auto-teste para fixação do conhecimento
→ [[2026-03-24_aprender-com-claude-6-prompts]]

### Curadoria de cursos gratuitos sobre Claude
[[usama-akram]] lista 5 cursos no YouTube:
1. Nick Saarev — Claude Code Full Course (4h): setup, subagentes, MCP
2. KJ Rainey — Claude Cowork: Zero to Working AI Employee
3. freeCodeCamp — Claude Code Essentials (certificado ExamPro)
4. Greg Isenberg — Building AI Agents that actually work
5. Brock Mesarich — Claude Cowork Ultimate Course
→ [[2026-04-05_5-videos-especialista-claude]]

### Curadoria de YouTubers para seguir continuamente
[[paul-hilse]] lista 7 canais com ângulos distintos: **Ben AI** (automação), **AI Edge** (no-code), **MIT Monk** (estratégia + IA + crescimento pessoal), **Jeff Su** (ferramentas no trabalho), **Dan Martell** (escalar negócio), **Varun Mayya** (impacto econômico da IA), **Liam Ottley** (AI agency). Complementa [[usama-akram]]: lá era "cursos fixos" (curto prazo), aqui é "pessoas para seguir" (longo prazo).
→ [[2026-04-22_7-youtubers-ia]]

## Fontes

- [[2026-03-24_aprender-com-claude-6-prompts]]
- [[2026-04-05_5-videos-especialista-claude]]
- [[2026-04-22_7-youtubers-ia]]
